...
__device__ inline uint64 copyBits(uint32 from, uint32 count, uint64 to, uint32 offset) {
...
template __device__ UArchStepStatus uarch_step(uarch_step_state_access &a);
